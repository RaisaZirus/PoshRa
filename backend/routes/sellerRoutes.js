...
// Explicit transaction: update seller_order status
  ...
  try {
    ...
    const { rows } = await client.query(
      `UPDATE seller_orders SET status = $1
       ...
       RETURNING seller_order_id, order_id`,
    ...
    // If all seller_orders for the master order are delivered → mark master as delivered
    if (status === "delivered") {
      await client.query(
...
// Explicit transaction: get/create courier → insert shipment → update order status
router.post("/shipments", async (req, res) => {
  const { seller_order_id, tracking_number, courier_name } = req.body;
  if (!seller_order_id) {
    return res.status(400).json({ success: false, message: "seller_order_id required" });
  }
...
// Complex query: joins products → stores → variants → images with aggregation
...
// Explicit transaction: update product fields
...
// Explicit transaction: insert payout row
...
// Explicit transaction: insert store row
router.post("/stores", async (req, res) => {
  ...
  try {
    ...
    await client.query("ROLLBACK");
    if (err.code === "23505") {
      return res.status(409).json({ success: false, message: "Store slug already taken" });
    }
...
// Explicit transaction: update stock/price — trg_prevent_negative_stock will fire
router.patch("/inventory/:variant_id", async (req, res) => {
  ...
  try {
    ...
    // Verify the variant belongs to this seller before updating
    const { rows: check } = await client.query(
      ...
      [req.params.variant_id, sellerId]
    );
    if (!check.length) {
      await client.query("ROLLBACK");
    ...
    await client.query(
      `UPDATE product_variants SET ${updates.join(", ")} WHERE variant_id = $1`,
      params
    );
    ...
    await client.query("ROLLBACK");
    // Surface the negative-stock trigger error clearly
    if (err.message?.includes("Stock cannot go below 0")) {
...
// Explicit transaction: insert answer
...
// Explicit transaction: update return status → optionally restore stock + recalculate subtotal
