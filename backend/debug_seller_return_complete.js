...
async function run() {
  ...
  try {
    ...
    console.log('retRows', retRows);
    if (!retRows.length) throw new Error('Return request not found');
    const ret = retRows[0];

    await client.query(`UPDATE return_requests SET status = 'completed' WHERE return_id = $1`, [returnId]);
    ...
    const { rows: paymentRows } = await client.query(`SELECT payment_id FROM payments WHERE order_id = $1 LIMIT 1`, [ret.order_id]);
    ...
    const { rows: remaining } = await client.query(`SELECT COUNT(*)::int AS cnt FROM order_items WHERE seller_order_id = $1`, [ret.seller_order_id]);
